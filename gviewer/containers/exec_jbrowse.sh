...
then
	jbrowse add-assembly ${DATA_DIRECTORY}/${FILE} --target ${DATA_DIRECTORY}/jbrowse2 --load symlink
	jbrowse set-default-session --name ${FILE%%.*} --view LinearGenomeView --target ${DATA_DIRECTORY}/jbrowse2
else 
	jbrowse add-track ${DATA_DIRECTORY}/${FILE} --target ${DATA_DIRECTORY}/jbrowse2 --category=${file_type} --load symlink
fi
